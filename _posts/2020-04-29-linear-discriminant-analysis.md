...
{% include image.html img="/assets/images/posts/LDA/intuition.png" desc="Deciding where a point is with respect to a line." %}
...
**Rule:** If $$\vt{(\bar{x}_1 - \bar{x}_2)} \mathbb{S}^{-1} \mathbf{x} > \frac{1}{2} \vt{(\bar{x}_1 - \bar{x}_2)} \mathbb{S}^{-1} \mathbf{(\bar{x}_1 + \bar{x}_2)}$$

Where,
$$\bar{x}_i = \frac{1}{n_i}\sum_{j = 1}^{j = n_i} \mathbf{x_j}^{(i)}$$
$$(n_1 + n_2 - 2) \mathbb{S} = (n_1 - 1) \mathbb{S}_1 + (n_2 - 1)\mathbb{S_2}$$
$$\mathbb{S}_i = \frac{1}{n_i - 1} \sum_{j = 1}^{j = n_i} \mathbf{(x_j - \bar{x}_i)} \vt{(x_j - \bar{x}_i)}$$
...
{% include image.html img="/assets/images/posts/LDA/fit.png" desc="The line created here seems to satisfy our earlier intuition." %}
